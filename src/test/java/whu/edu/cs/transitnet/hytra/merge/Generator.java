package whu.edu.cs.transitnet.hytra.merge;
...
public class Generator {
     ...
     */
    public static HashMap<String, HashMap<Integer, HashSet<String>>> planes = new HashMap<>();
    ...
    public static void generateMap() {
        ...
        System.out.println("merged to " + sum);


    }

    public static HashMap generatePlanes() {
            ...
            for(int i = 0; i < size; i++){
                if (cubes[i] == 1){
                    ...
                    for (int a = box[0]; a <= box[1]; a++){
                        if(!planes.get(day).containsKey(a)){planes.get(day).put(a, new HashSet<>());}
                        planes.get(day).get(a).add(cid);
                    }
                    for (int b = box[2]; b <= box[3]; b++){
                        int idx = b + (int) Math.pow(2, resolution);
                        if(!planes.get(day).containsKey(idx)){planes.get(day).put(idx, new HashSet<>());}
                        planes.get(day).get(idx).add(cid);
                    }
